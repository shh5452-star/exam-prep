...
export default function QuizApp() {
  ...
  useEffect(() => {
    ...
    setTip(0);
    const id = setInterval(() => setTip((p) => p + 1), 1600);
    return () => clearInterval(id);
  }, [phase]);

  const answeredCount = useMemo(
    () => activeQuestions.filter((_, i) => (answers[i] ?? "").trim() !== "").length,
  ...
  function startRound(qs: Question[], review: boolean) {
    ...
    setIsReviewRound(review); setPhase("quiz");
  ...
  async function handleFile(file: File) {
    ...
    try {
      if (ext === "pdf") {
        ...
        if (text.trim().length < 10) { setErrorMsg("파일 내용이 너무 짧아요."); setExtracting(false); return; }
        setSourceText(text); setPdfFile(null);
      }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (pdfFile) {
        ...
        res = await createQuiz(title.trim() || "제목 없음", sourceText, count, difficulty);
      }
      if (!res.questions.length) { setErrorMsg("문제를 만들지 못했어요. 다시 시도해 주세요."); setPhase("input"); return; }
      setSummary(res.summary); setAllQuestions(res.questions);
      startRound(res.questions, false);
  ...
  function handleGrade() {
    ...
    let score = 0;
    activeQuestions.forEach((q, i) => { if (isCorrect(q, answers[i])) score++; });
    setResult({ score, total: activeQuestions.length });
    setConfirmGrade(false); setPhase("result");
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
  ...
  async function handleSimilar() {
    ...
    setLoadingKind("similar"); setPhase("loading");
    try {
      let res: QuizResult;
      if (pdfFile) {
        const fd = new FormData();
        fd.append("file", pdfFile);
        fd.append("difficulty", difficulty);
        fd.append("weak", JSON.stringify(weak));
        res = await createSimilarQuizFromPdf(fd);
      } else {
        res = await createSimilarQuiz(sourceText, weak, difficulty);
      }
      if (!res.questions.length) { setErrorMsg("비슷한 문제를 만들지 못했어요."); setPhase("result"); return; }
      if (res.summary) setSummary(res.summary);
      startRound(res.questions, true);
  ...
  function handleSave() {
    ...
    const entry: SavedQuiz = {
      id: Date.now().toString(), title: title || "제목 없음", date: new Date().toISOString(),
      score: result.score, total: result.total, summary, sourceText, difficulty,
      questions: activeQuestions, answers,
    };
    const updated = [entry, ...history].slice(0, MAX_SAVED);
    setHistory(updated); saveHistoryToStorage(updated); setSavedCurrent(true);
  }
  function handleLoadSaved(saved: SavedQuiz) {
    setTitle(saved.title); setSourceText(saved.sourceText); setDifficulty(saved.difficulty);
  ...
  function handleDeleteSaved(id: string) {
    ...
    setHistory(updated); saveHistoryToStorage(updated);
  }
  function resetAll() {
        ...
        {phase === "input" && (
          <div className="space-y-5">
            <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()}
              className={`flex cursor-pointer flex-col items-center rounded-2xl border-2 border-dashed p-6 transition ${dragOver ? "border-[#7C9CF5] bg-[#1A2030]" : "border-[#2A2E37] bg-[#1D2026] hover:border-[#3A4150]"}`}>
              ...
              {extracting ? (
                <div className="flex items-center gap-3">
                  <div className="h-5 w-5 animate-spin rounded-full border-2 border-[#2A2E37] border-t-[#7C9CF5]" />
                  <span className="text-sm text-[#8A92A0]">파일 처리 중...</span>
                </div>
              ) : pdfFile ? (
                <div className="text-center">
                  <p className="text-sm text-[#5ED3A8]">✓ PDF 업로드 완료</p>
                  <p className="mt-1 text-xs text-[#8A92A0]">📄 {fileName}</p>
                  <p className="mt-1 text-xs text-[#6B7280]">AI가 PDF를 직접 읽고 문제를 만들어요</p>
                </div>
              ) : fileName ? (
                <p className="text-sm text-[#7C9CF5]">📄 {fileName}</p>
              ) : (
                <><p className="text-[#8A92A0]">PDF, TXT 파일을 여기에 놓거나 클릭</p><p className="mt-1 text-xs text-[#6B7280]">또는 아래에 직접 붙여넣기</p></>
              )}
            </div>
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="제목 (예: 운영체제 3주차)"
              className="w-full rounded-xl border border-[#2A2E37] bg-[#1D2026] px-4 py-3 text-[#F4F6F9] placeholder-[#6B7280] outline-none focus:border-[#7C9CF5]" />
            {!pdfFile && (
              <textarea value={sourceText} onChange={(e) => setSourceText(e.target.value)} placeholder="여기에 강의 자료를 붙여넣으세요..." rows={9}
            ...
            <div className="flex flex-wrap gap-8">
              <div><p className="mb-2 text-sm text-[#8A92A0]">문항 수</p><div className="flex gap-2">
                {COUNTS.map((c) => (<button key={c} onClick={() => setCount(c)} className={`rounded-lg border px-4 py-2 text-sm transition ${count === c ? "border-[#7C9CF5] bg-[#233049] text-[#B9CBFF]" : "border-[#2A2E37] bg-[#1D2026] text-[#C7CCD4] hover:border-[#3A4150]"}`}>{c}개</button>))}
              </div></div>
              <div><p className="mb-2 text-sm text-[#8A92A0]">난이도</p><div className="flex gap-2">
                {DIFFICULTIES.map((d) => (<button key={d.value} onClick={() => setDifficulty(d.value)} className={`rounded-lg border px-4 py-2 text-sm transition ${difficulty === d.value ? "border-[#7C9CF5] bg-[#233049] text-[#B9CBFF]" : "border-[#2A2E37] bg-[#1D2026] text-[#C7CCD4] hover:border-[#3A4150]"}`}>{d.label}</button>))}
              </div></div>
            </div>
            {errorMsg && <p className="text-sm text-[#E5746B]">{errorMsg}</p>}
            <button onClick={handleCreate} className="w-full rounded-xl bg-[#7C9CF5] py-3.5 font-medium text-[#15171C] transition hover:bg-[#93AEF7] active:scale-[0.99]">예상문제 만들기</button>
            {history.length > 0 && (
              <div className="pt-4">
                ...
                {showHistory && (<div className="mt-3 space-y-2">{history.map((h) => (
        ...
        {phase === "quiz" && (
          <div className="space-y-5">
            <div>
              ...
              <div className="h-1.5 w-full rounded-full bg-[#2A2E37]"><div className="h-1.5 rounded-full bg-[#7C9CF5] transition-all duration-300" style={{ width: `${progress}%` }} /></div>
            </div>
            {activeQuestions.map((q, i) => (
              <div key={i} className="rounded-2xl border border-[#2A2E37] bg-[#1D2026] p-5">
                ...
                {q.type === "mcq" && q.choices ? (
                  <div className="space-y-2">{q.choices.map((c) => (
                    <button key={c} onClick={() => setAnswers({ ...answers, [i]: c })}
                      className={`block w-full rounded-xl border px-4 py-2.5 text-left text-[15px] transition ${answers[i] === c ? "border-[#7C9CF5] bg-[#233049] text-[#B9CBFF]" : "border-[#2D323C] bg-[#20242B] text-[#C7CCD4] hover:border-[#3A4150]"}`}>{c}</button>
                  ))}</div>
                ) : (
                  <input value={answers[i] ?? ""} onChange={(e) => setAnswers({ ...answers, [i]: e.target.value })} placeholder="답을 입력하세요"
                    className="w-full rounded-xl border border-[#2D323C] bg-[#20242B] px-4 py-2.5 text-[#E7E9ED] placeholder-[#6B7280] outline-none focus:border-[#7C9CF5]" />
                )}
              </div>
            ))}
            {confirmGrade && remaining > 0 && <p className="text-sm text-[#E0A458]">아직 안 푼 문제가 {remaining}개 있어요. 그래도 채점하려면 한 번 더 누르세요.</p>}
            <button onClick={handleGrade} className="w-full rounded-xl bg-[#7C9CF5] py-3.5 font-medium text-[#15171C] transition hover:bg-[#93AEF7] active:scale-[0.99]">{remaining > 0 ? `안 푼 문제 ${remaining}개 · 채점하기` : "채점하기"}</button>
          </div>
        )}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col items-center rounded-2xl border border-[#2A2E37] bg-[#1D2026] py-8">
        ...
        <button onClick={onSave} disabled={savedCurrent} className={`mt-5 rounded-xl px-5 py-2.5 text-sm font-medium transition ${savedCurrent ? "bg-[#233049] text-[#6B7280]" : "bg-[#2D323C] text-[#B9CBFF] hover:bg-[#3A4150]"}`}>{savedCurrent ? "✓ 저장됨" : "이 결과 저장하기"}</button>
      </div>
      {wrongCount > 0 && !similarDismissed && (
        <div className="rounded-2xl border-2 border-[#7C9CF5] bg-[#1A2030] p-5">
          ...
          <div className="mt-4 flex gap-3">
            <button onClick={onSimilar} className="rounded-xl bg-[#7C9CF5] px-5 py-2.5 font-medium text-[#15171C] transition hover:bg-[#93AEF7] active:scale-[0.99]">예, 만들어 주세요</button>
      ...
      {wrongCount > 0 ? (
        <div><p className="mb-3 text-sm text-[#8A92A0]">오답노트 ({wrongCount})</p><div className="space-y-3">
          {questions.map((q, i) => isCorrect(q, answers[i]) ? null : (
